...
# Approach
The idea is to delay running the function using setTimeout and store the returned timeout ID. Then I created a cancel function that calls clearTimeout with that ID so it can stop the function from running if called early. I then returned the cancelFn so it can be invoked later to prevent execution.
